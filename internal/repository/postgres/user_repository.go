...
func (r *userRepository) CreateUser(ctx context.Context, user *domain.User) error {
	...
	_, err := r.db.ExecContext(ctx, query, user.ID, user.FirstName, user.LastName, user.Nickname, user.Password, user.Email, user.Country, user.CreatedAt, user.UpdatedAt)
	return err
}

// UpdateUser updates an existing user in the database.
func (r *userRepository) UpdateUser(ctx context.Context, user *domain.User) error {
	query := `UPDATE users SET first_name = $1, last_name = $2, nickname = $3, password = $4, email = $5, country = $6, updated_at = $7 WHERE id = $8`
	_, err := r.db.ExecContext(ctx, query, user.FirstName, user.LastName, user.Nickname, user.Password, user.Email, user.Country, user.UpdatedAt, user.ID)
	return err
}
...
func (r *userRepository) GetUserByID(ctx context.Context, userID uuid.UUID) (*domain.User, error) {
	query := `SELECT id, first_name, last_name, nickname, password, email, country, created_at, updated_at FROM users WHERE id = $1`
	row := r.db.QueryRowContext(ctx, query, userID)

	var user domain.User
	err := row.Scan(&user.ID, &user.FirstName, &user.LastName, &user.Nickname, &user.Password, &user.Email, &user.Country, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
